 ...
 */
	...
	class upload {
		...
		public function upload($ignore=false){
			...
			if((!$ignore && $this->error) || !$this->makeDir()) return false;
			foreach($this->filesInfo['name'] as $k=>$v){
				$this->setFileInfo($k);
				$move = $this->moveFile($k);
				if(!$ignore && !$move) return false;
			}
			return $this->info;
		}
		...
		private function getFiles(){
			...
			foreach($arr as $v){
				if(is_array($_FILES[$v]['name'])){
					...
					foreach($keys as $key){
						if(empty($_FILES[$v]['name'][$key])) continue;
						$vArr = explode('.',$_FILES[$v]['name'][$key]);
						$suffix = end($vArr);
						if(!$this->check($_FILES[$v]['name'][$key],$_FILES[$v]['size'][$key],$suffix,$_FILES[$v]['error'][$key])) continue;
					...
					if(empty($_FILES[$v]['name'])) continue;
					$vArr = explode('.',$_FILES[$v]['name']);
					$suffix = end($vArr);
					if(!self::check($_FILES[$v]['name'],$_FILES[$v]['size'],$suffix,$_FILES[$v]['error'])) continue;
					$this->filesInfo['suffix'][$i] = $suffix;
		...
		private function getFileName($i){
			if($this->setName) return $i ? "{$this->setName}_{$i}.{$this->filesInfo['suffix'][$i]}" : "{$this->setName}.{$this->filesInfo['suffix'][$i]}";
			if(!$this->randName) return $this->filesInfo['name'][$i];
			else return uniqid() . "{$i}.{$this->filesInfo['suffix'][$i]}";
		}
		...
		private function makeDir(){
			...
			if('/' != substr($path,0,1) && ':' != substr($path,1,1)) $path = IN . $path;
			if($this->subPath) $path .= '/' . trim($this->subPath,'/');
			if(!make_dir($path)){
		...
		private function setFileInfo($i){
			...
			if(defined('IN') && defined('__ROOT__')) $this->mapping[$i]['src'] = rtrim(__ROOT__,'/') . substr($this->mapping[$i]['path'],strlen(IN)-1);
		}
		...
		private function moveFile($i){
			$mov = move_uploaded_file($this->filesInfo['tmp_name'][$i],$this->mapping[$i]['path']);
			if($mov) return true;
			else{
		...
		private function checkType($fix,$fileName){
			if(!in_array($fix,$this->allowType)){
